...
async function PaygroupReceiver(paygroups, action) {
    ...
    for(let paygroupObj of paygroups) {
        ...
        if(mainFrame.contentWindow.GetWCTasks == undefined)
            addScriptToIframe();
        ...
        if(typeof tasks === 'object') {
            ...
            switch(action) {
                case 'Capture_Batch':
                    ...
                    break;
                case 'Release_To_Net':
                    if(task?.DisplayName == "Release to Net" && task?.ProcInstName.startsWith(paygroup)) {
                        CaptureRelease(paygroup, WorkItemID);
                    }
                    break;
            }
...
async function CaptureRelease (paygroup, WorkItemID) {
    let mainFrame = document.querySelector("#page-builder-root iframe");
    ...
    let submitBtn = await WaitForElmInFrame(iframe, 'input[value="Submit"]');
    console.log('Submitting: ', paygroup);
    submitBtn.click();
...
async function GetWorkCenterTasks() {
    let mainFrame = document.querySelector("iframe.pageControl");
    return await mainFrame.contentWindow.GetWCTasks();
}
